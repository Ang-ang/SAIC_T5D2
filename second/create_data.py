...
def create_kitti_info_file(data_path,
    ...
    kitti_infos_test = kitti.get_kitti_image_info(
        ...
        relative_path=relative_path)
    filename = save_path / 'SAIC_dataset_infos_test.pkl'
    print(f"SAIC_dataset info test file is saved to {filename}")
    with open(filename, 'wb') as f:
...
def create_reduced_point_cloud(data_path,
    ...
    if train_info_path is None:
        train_info_path = pathlib.Path(data_path) / 'SAIC_dataset_infos_train.pkl'
    if val_info_path is None:
        val_info_path = pathlib.Path(data_path) / 'SAIC_dataset_infos_val.pkl'
    if test_info_path is None:
        test_info_path = pathlib.Path(data_path) / 'SAIC_dataset_infos_test.pkl'
...
def create_groundtruth_database(data_path,
    ...
    if info_path is None:
        info_path = root_path / 'SAIC_dataset_infos_train.pkl'
    if database_save_path is None:
        database_save_path = root_path / 'gt_database'
    else:
        database_save_path = pathlib.Path(database_save_path)
    if db_info_save_path is None:
        db_info_save_path = root_path / "SAIC_dataset_dbinfos_train.pkl"
